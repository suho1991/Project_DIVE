	...
	<div class="product_list_wraping">
		...
		<div class="product_item">
			<div class="product_user">
				...
				<div class="product_region">${product.region}</div>
			</div>
			<!-- 신뢰도 bar-->
			<div class="reliability">
				<div class="meter">
					<span style="width: 30%;"> <span class="progress"></span>
					</span>
				</div>
				<h5>매너온도</h5>
			</div>
			...
			<div class="product_content_top">
				<div class="product_describe">
					...
					<p class="product_date">
						<fmt:formatDate pattern="yyyy-MM-dd" value="${product.updateDate}"></fmt:formatDate>
					</p>
				...
				<div class="chat_btn_area">
					<a
						onclick="window.open ( '/chat/chatting', 'name', 'width = 400, height = 569, scrollbars=no, resizable=no, toolbars=no, menubar=no')">
						<input type="button" id="enterBtn" value="채팅하기" />
					</a>
				</div>

			</div>

			<p class="product_content_main">${product.description}</p>

			<div class="product_content_bottom">
				<div class="product_interest">채팅 1 ∙ 관심 ${product.likeCount} ∙
					조회 ${product.hitsCount}</div>
...
<form id="operForm" action="/product/updateProduct" method="get">
	<input type="hidden" id="id" name="id" value="${product.id}"> <input
		type="hidden" name="userNum" value="${product.userNum}"> <input
		type="hidden" id="pageNum" name="pageNum" value="${criteria.pageNum}">
	<input type="hidden" id="amount" name="amount"
		value="${criteria.amount}"> <input type="hidden" name="type"
		value="${criteria.type}"> <input type="hidden" name="keyword"
		value="${criteria.keyword}"> <input type='hidden'
	...
	$(document)
			.ready(
					function() {
														...
														.each(
																function(i,
																	...
																	var fileCallPath = encodeURIComponent(attachVO.uploadPath
																			+ "/"
																			+ attachVO.thumbnailHeader
																			+ attachVO.uuid
																			+ attachVO.fileNameSeparatorUuid
																			+ attachVO.fileName);
																	if (attachVO.fileType) {
																		outStr += "<li class='slide'>"
																				+ "<img class='productImg' src='/displayImage?absoluteFileName="
																				+ fileCallPath
																				+ "' >"
																				+ "</li>";
																	}
						...
						$("button[data-oper='update']").on("click",
								function(e) {
									operForm.submit();
								});
